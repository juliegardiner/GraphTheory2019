...
def shunt(infix):
    """The Shunting Yard Algorithm for Converting Regular Expressions to Postfix."""
...
def match(infix, string):
    ...
    #List containing infix regular expressions
infixes = ["a.b.c*", "a(b|d).c", "(a(b.b)*.c)"]

    #List of Strings to match against the infix regular expressions
strings = [" ","abc","abbc","abcc","abab","abbbc"]

    #for loop loops through infixes(i) followed by loop through strings(j) and print the matches
for i in infixes:
    for s in strings:
        print(match(i,s), i,s)
